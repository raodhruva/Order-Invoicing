<script type="text/javascript">
	...
	function dataInit(){
		//make sure order number is changed
		alert("please make sure the order number is changed");
    ...
    editform.numberOfRows.value = sno;
    subtotal += total;
    CalculateTotal();
};
...
//this array will be used to update quantity
$differenceValues = json_decode($_GET['differenceValues']);
...
if ($tableData = $con-> query($displayOrderSpecs)) {
	...
	while($row = $tableData-> fetch_assoc()) {
		...
		//$total = $row["total"];
			...
			$total = $differenceValues[$sno - 1] * $price;

		// just replace quantity with difference values from previous page
			echo '<tr>
			<td><input type="number" id="tqty'.$sno.'" name = "tqty'.$sno.'" value = "'.$differenceValues[$sno - 1].'"></td>
